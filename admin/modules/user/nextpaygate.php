...
$page->add_breadcrumb_item("پرداخت آنلاين نکست پی", "index.php?module=user-nextpaygate");
...
if($mybb->input['action'] == "edit_pack")
{
	$sub_tabs['edit_pack'] = array(
		'title' => 'ویرایش بسته',
		'link' => "index.php?module=user-nextpaygate&amp;action=edit_pack",
	...
	$sub_tabs['manual_add'] = array(
		...
		'link' => "index.php?module=user-nextpaygate&amp;action=manual_add",
	...
	$sub_tabs['tractions'] = array(
		...
		'link' => "index.php?module=user-nextpaygate&amp;action=tractions",
...
$d = "سال";
		...
		$pname=$over['title'];
		$table->construct_cell("<strong><a href=\"index.php?module=user-nextpaygate&amp;action=edit_pack&amp;num={$over['num']}\">{$over['title']}</a></strong></td><td>{$over['description']}</td><td>{$over['period']} {$d}  </td><td>{$over['price']} تومان</td>");
		$popup = new PopupMenu("sub_{$over['num']}", $lang->options);
		$popup->add_item($lang->delete, "index.php?module=user-nextpaygate&amp;action=delete_pack&amp;num={$over['num']}");
		$popup->add_item($lang->edit, "index.php?module=user-nextpaygate&amp;action=edit_pack&amp;num={$over['num']}");
	...
	$querypage = $db->simple_select("npgate", "COUNT(num) as packs", "1=1");
	$total_rows = $db->fetch_field($querypage, "packs");
			
	$pagination = draw_admin_pagination($mybb->input['page'], $per_page, $total_rows, "index.php?module=user-nextpaygate&amp;page={page}");
	echo $pagination;					
...
$db->insert_query("npgate", $insert_query);
					flash_message('بسته ی مورد نظر با موفقیت ذخیره شد', 'success');
					admin_redirect("index.php?module=user-nextpaygate");
					}
}
...
// Time
		...
		$form = new Form("index.php?module=user-nextpaygate&amp;action=add_pack", "post");
		if($errors)
	{
...
'period' => intval($mybb->input['period']),
					...
					$db->update_query('npgate', $update_array, 'num=\''.intval($num).'\'');
...
//
    ...
    $num = $mybb->input['num'];
	$query = $db->simple_select("npgate","*","num='$num'");
    $edit = $db->fetch_array($query);

	$form = new Form("index.php?module=user-nextpaygate&amp;action=edit_pack&amp;num={$num}", "post");
			if($errors)
	{
		$page->output_inline_error($errors);
	}
...
{
$num=intval($mybb->input['num']);
$db->query("DELETE FROM ".TABLE_PREFIX."npgate WHERE num=$num");
...
//\End Time Filter
	...
	//\Filter
		...
		$table->construct_header("گزینه‌ها", array('class' => 'align_left', width => '7%'));
			...
			$query4 = $db->simple_select('npgate', 'num, title', '', array('order_by' => 'num', 'order_dir' => 'asc'));
			while($pack = $db->fetch_array($query4, 'num, title'))
			{
				$packs[$pack['num']] = $pack['title'];
			}
		$query5 = $db->query("SELECT l.*
		FROM ".TABLE_PREFIX."npgate_tractions l
...
if($track['stauts'] != 0)
{
	...
	$querypage = $db->simple_select("npgate_tractions l", "COUNT(l.tid) as tracs", "1=1 {$additional_sql_criteria}");
	$total_rows = $db->fetch_field($querypage, "tracs");
	$pagination = draw_admin_pagination($mybb->input['page'], $per_page, $total_rows, "index.php?module=user-nextpaygate&amp;action=tractions&amp;page={page}{$additional_criteria}");
				echo $pagination;
				echo "<div class = \"float_left\"><a href=\"index.php?module=user-nextpaygate&amp;action=deletealltrac\">حذف تمامی تراکنش‌های پایان یافته</a></div>";
	...
	$packquery = $db->simple_select("npgate", "num,title", "1=1");
	...
	$neper['1']="قبل";
    $neper['2']="بعد";
	$form = new Form("index.php?module=user-nextpaygate&amp;action=tractions", "post");
...
$query = $db->simple_select("npgate_tractions", "*", "tid = $tid");
  ...
  'usergroup' => $pgid
                ...
                $db->update_query("npgate_tractions", $update_array, "`tid` = '$tid'");
...
$uid = $user['uid'];
$pgid = $user['usergroup'];
$queryp = $db->simple_select("npgate", "*", "num={$packid}");
...
$querychk = $db->simple_select("npgate_tractions", "stauts", "uid='{$uid}'");
...
//Add TR
$db->insert_query("npgate_tractions", $insert_query);
//Change UG
...
//Message
	...
	$form = new Form("index.php?module=user-nextpaygate&amp;action=manual_add", "post");
		if($errors)
	{
		$page->output_inline_error($errors);
	}
